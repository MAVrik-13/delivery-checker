...
def _normalize_city_name(address: str) -> str:
    """Нормализует название города в адресе (расширение сокращений)."""
    ...
    # Разбиваем адрес на слова и заменяем сокращения
    words = address.split()
    normalized_words = []
    
    for word in words:
        lower_word = word.lower()
        # Проверяем, является ли слово сокращением города
        if lower_word in city_abbreviations:
            normalized_words.append(city_abbreviations[lower_word])
        else:
...
def geocode_address(address: str) -> Optional[dict]:
    ...
    # ── Нормализация адреса: расширение сокращений городов ──
    address_normalized = _normalize_city_name(address)

    # ── Провайдер 1: Photon (komoot) — работает без ключа ──
    try:
        resp = requests.get(
            "https://photon.komoot.io/api/",
            params={"q": address_normalized, "limit": 1},
